...
class Mev(Dataset):
    ...
    def __init__(self):
        super(Mev, self).__init__()
        self.name = self.config.get('mev', 'name')
        self.url = self.config.get('mev', 'url')
    ...
    def download_data(self):
        ...
        filename = self.config.get('mev', 'filename')
    ...
    def build_data(self):
        ...
        print('running R mev transformation ...')
        ...
        Mev_r_file = working_directory + self.config.get('mev', 'mev_r_file')
        input_r_file = self.config.get('mev', 'input_file_for_R')
        output_r_file = self.config.get('mev', 'output_file_for_R')

        command = rscript, Mev_r_file, working_directory, input_r_file, output_r_file

        print(command)
        print(subprocess.check_output(command))

        print('successfully completed R mev transformation')
...
def main():
    dataset = Mev()
    Mev.download_data(dataset)
    Mev.build_data(dataset)
